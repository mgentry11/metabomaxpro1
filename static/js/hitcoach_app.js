...
function resetExerciseUI() {
    ...
    startBtn.style.backgroundColor = ''; // Reset to CSS default
    startBtn.style.borderColor = '';
    startBtn.onclick = function() { startExercise(); };
...
function getLastWeight(exerciseName) {
    // First try the new exercise history format
    const exerciseHistory = JSON.parse(localStorage.getItem('exerciseHistory')) || [];
    for (let i = exerciseHistory.length - 1; i >= 0; i--) {
        if (exerciseHistory[i].exerciseName === exerciseName) {
            return exerciseHistory[i].weight;
        }
    }

    // Fall back to workout history if needed
...
function startExercise() {
    ...
    // Track exercise start time for duration calculation
...
function finishExercise() {
    ...
    // Change START button to COMPLETE EXERCISE and style it
    const startBtn = document.getElementById('startBtn');
    startBtn.textContent = 'COMPLETE EXERCISE';
    startBtn.style.display = 'block';
    startBtn.style.backgroundColor = '#10b981'; // Success green
    startBtn.style.borderColor = '#10b981';
    startBtn.onclick = function() { showCompletionUI(); };

    // If auto-complete is enabled, automatically complete the exercise
    if (settings.autoComplete) {
        setTimeout(() => {
            autoCompleteExercise();
        }, 1000); // Give user 1 second to see the completion state
    }
}

function showCompletionUI() {
    // Hide the green COMPLETE EXERCISE button
    document.getElementById('startBtn').style.display = 'none';

    // Show the completion section
    document.getElementById('completeSection').style.display = 'block';

    // Ensure weight input section is visible if weight not entered
    const weightInput = document.getElementById('weightInput');
    if (!weightInput.value || weightInput.value === '0') {
        document.querySelector('.weight-input-section').style.display = 'block';
    }
}
...
function autoCompleteExercise() {
    ...
    if (weight === 0) {
        // Can't auto-complete without weight, show completion UI
        showCompletionUI();
        speak('Please enter the weight used');
        return;
    }

    // Auto-complete assumes failure was reached (since timer completed)
    document.getElementById('failureCheck').checked = true;
    completeExercise();
}
...
function completeExercise() {
    ...
    const duration = exerciseStartTime ? Math.floor((new Date() - exerciseStartTime) / 1000) : 0;
    ...
    // Save exercise data with enhanced structure
    const exerciseData = {
    ...
    // Also save individual exercise to localStorage for quick access
    saveIndividualExerciseData(exerciseData);
...
function saveIndividualExerciseData(exerciseData) {
    try {
        // Get existing exercise history
        const exerciseHistory = JSON.parse(localStorage.getItem('exerciseHistory')) || [];
        ...
        if (exerciseHistory.length > 1000) {
...
function shareWorkout() {
    const exercises = currentWorkoutData.map(ex => {
        ...
        return `${exerciseName}: ${ex.weight} lbs${ex.failure ? ' ✓' : ''}`;
    }).join('\n');
...
function loadProgressHistory() {
    ...
    history.reverse().forEach((workout, index) => {
        ...
        workout.exercises.forEach(ex => {
            ...
            const exerciseName = ex.exerciseName || ex.name;
            const durationStr = ex.duration ? ` (${formatTime(ex.duration)})` : '';

            exercisesHTML += `
                <div class="progress-exercise">
                    <span class="progress-exercise-name">${exerciseName}${durationStr}</span>
                    <div>
